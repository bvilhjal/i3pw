...
\textbf{Review scope.}
This report describes the local version-0.3.1 worktree reviewed on the date above.
...
The reviewed source declares version 0.3.1 and an alpha development status.  Its
...
validation study.  Figure~\ref{fig:fresh-benchmarks} visualizes the three contrasts
that are most useful for interpreting those checks.
...
\end{itemize}
The risks are clear: marginal prevalences cannot identify a joint distribution;
exact anchors are tautologies; weak support creates extreme weights; and validation
remains synthetic.  The next milestone is a prespecified external-cohort or
held-out-register study of quantities withheld from fitting.
