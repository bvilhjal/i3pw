...
| `π(x, y) = P(S = 1 \| X = x, Y = y)` | the true, unknown inclusion probability | never observed |
| `K_q = P(Y_q = 1)` | **known** population prevalence | a register or census |
| `P_q = P(Y_q = 1 \| S = 1)` | sample prevalence — biased | the cohort |
| `g(Y)` | the constrained functions: `Y` itself, or `Y` crossed with stratum indicators | your choice — the design decision |
| `d_i` | base weights, typically `1 / π̂(X_i)` | your participation model (`base_weights=`) |
...
`λ` and assigns weights proportional to `exp(λ·Y)`. The constraint is that the
weighted case fraction equal `K`:

```
    P·exp(λ)                                odds(K)         0.40/0.60
------------------- = K    =>    λ = log --------- = log ----------- = 0.9808
P·exp(λ) + (1 − P)                        odds(P)         0.20/0.80
```
...
```
w_i  ∝  d(X_i) · exp( λ·g(Y_i) )
```

— the base `d(X)` tilted by the smallest exponential factor — that reproduce the supplied
population moments `E_w[g(Y)] = target`. This is a *density-ratio* model,
...
- **An exponential-tilt density-ratio model.** Writing
  `log dP_pop/dP_sample = a(X) + θ·g(Y)` is
...
```
primal — n unknowns                 dual — k unknowns

min_w   Σ_i w_i log(w_i / d_i)      min_λ   log Σ_i d_i exp( λ·(g(Y_i) − t) ) + (ρ/2)·||λ||²
s.t.    Σ_i w_i g(Y_i) = t
        Σ_i w_i = 1                 then    w_i ∝ d_i exp( λ·g(Y_i) )
```
...
deliberately keeps separate — one per term of `a(X) + θ·g(Y)`:
